...
TEST_F(PsaCipherEncrypt, BadState) {
    ...
    psa_status_t status = psa_cipher_encrypt(1, PSA_ALG_CTR,
			msg, sizeof(msg),
			cipher_buf, sizeof(cipher_buf), &output_length);
...
TEST_F(PsaCipherEncrypt, InvalidAlgorithm) {
    ...
    status = psa_cipher_encrypt(key_handle, PSA_ALG_XTS,
            msg, sizeof(msg),
            cipher_buf, sizeof(cipher_buf), &output_length);
...
TEST_F(PsaCipherEncrypt, InvalidOutputSize) {
    ...
    psa_status_t status = psa_import_key(&key_attributes, aes_ctr_key, sizeof(aes_ctr_key), &key_handle);
    EXPECT_EQ(status, PSA_SUCCESS);
    status = psa_cipher_encrypt(key_handle, PSA_ALG_CTR,
            msg, sizeof(msg),
            cipher_buf, 79, &output_length);
...
TEST_F(PsaCipherEncrypt, Success) {
    ...
    status = psa_cipher_decrypt(key_handle, PSA_ALG_CTR,
            ...
            plain_buf, sizeof(plain_buf), &plain_length);
    EXPECT_EQ(status, PSA_SUCCESS);
