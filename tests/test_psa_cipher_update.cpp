...
class PsaCipherUpdate : public ::testing::Test {
    protected:
        void SetUp() override 
        {
            ...
            psa_status_t status = psa_import_key(&attributes, aes_ctr_key, sizeof(aes_ctr_key), &key_handle);
...
TEST_F(PsaCipherUpdate, NullInputArgument) {
    ...
    psa_status_t status = psa_cipher_update(&operation, NULL, sizeof(input), output, 16, &output_length);
...
TEST_F(PsaCipherUpdate, NullOutputArgument) {
    uint8_t input[16] = { 0 };
    size_t output_length = 0;
    psa_status_t status = psa_cipher_update(&operation, input, sizeof(input), NULL, 16, &output_length);
    EXPECT_EQ(status, PSA_ERROR_INVALID_ARGUMENT);
}
...
TEST_F(PsaCipherUpdate, InvalidOutputSize) {
    ...
    uint8_t output[15] = { 0 };
    size_t output_length = 0;
    psa_status_t status = psa_cipher_update(&operation, input, sizeof(input), output, 15, &output_length);
...
TEST_F(PsaCipherUpdate, Ok_SizeOfOutput_EqualToInput) {
    ...
    uint8_t output[16] = { 0 };
    size_t output_length = 0;
    psa_status_t status = psa_cipher_update(&operation, input, sizeof(input), output, sizeof(output), &output_length);
    EXPECT_EQ(status, PSA_SUCCESS);
...
TEST_F(PsaCipherUpdate, MultipleUpdates) {
    ...
    size_t output_length = 0;
    psa_status_t status = psa_cipher_update(&operation, input, block_size, output, sizeof(output), &output_length);
    EXPECT_EQ(status, PSA_SUCCESS);
    EXPECT_EQ(output_length, block_size);
    status = psa_cipher_update(&operation, input+block_size, block_size, output, sizeof(output)-block_size, &output_length);
    EXPECT_EQ(status, PSA_SUCCESS);
    EXPECT_EQ(output_length, block_size);
}