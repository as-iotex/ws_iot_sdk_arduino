...
void setup()
{
  ...
  Serial.println("This example shows how to use the W3bstream PSA Crypto API to generate random values.");
  Serial.println("Note: This example is only for demonstration purposes. Production applications should seed the random generator with enough entropy.");
  Serial.println("      PIN1 analog read is used as the entropy source, so the random generated values will be the same between boots unless the state of pin 1 changes.");
  Serial.println("--------------------------------------------------------------------\n");
  ...
  {
    if (random[i] < 0x10) Serial.print("0");
