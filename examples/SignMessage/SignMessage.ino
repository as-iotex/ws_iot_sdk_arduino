...
void setup() {
  ...
  Serial.println("This example shows how to use the W3bstream PSA Crypto API to sign a message using the Secp256r1 Elliptic Curve.");
  Serial.println("--------------------------------------------------------------------\n");
  ...
  psa_set_key_usage_flags(&keyAttributes, PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_EXPORT);
  ...
  status = psa_sign_message(keyId, PSA_ALG_ECDSA(PSA_ALG_SHA_256), message, sizeof(message), signature, sizeof(signature), &signatureLength);
  if (status != PSA_SUCCESS)
  {
  ...
  status = psa_verify_message(keyId, PSA_ALG_ECDSA(PSA_ALG_SHA_256), message, sizeof(message), signature, signatureLength);
  if (status != PSA_SUCCESS)
  {
